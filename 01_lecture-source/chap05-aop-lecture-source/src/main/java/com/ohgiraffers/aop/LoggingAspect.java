...
*    - Pointcut(실행 할 위치) 과 Advice(실행 할 위치에서 동작 할 코드) 를
*      하나의 클래스 단위로 정의하기 위한 어노테이션 */
@Aspect
@Component
public class LoggingAspect {
    ...
    *   * -> 와일드 카드(전부 가능하다는 의미)  */
    ...
    /* comment.
    *   JoinPoint : @Pointcut 으로 설정 한 실행 지점을 의미한다.
    *   실행 지점을 설정하게 되면 우리가 메소드 내에 작성한 코드(Advice) 를
    *   어노테이션의 종류에 따라 전, 후, 전후 등에 동작하게 할 수 있다.
    *   메소드의 매개변수로 전달 한 JoinPoint 객체는 현재 JoinPoint 의
    *   메소드명, 인수, 정보 등을 가지고 있게 된다.
    * */
    ...
    // 같은 클래스 내에 PointCut 을 설정했다면 클래스명 생략 가능
    ...
    *    = 리턴 할 결과 값을 변경 할 수 있다.
    *   returning 속성
    *    - 메소드가 동작을 완료하고 자신을 호출 한 쪽으로 가져가는 값
    *      즉 리턴값을 담아 둘 변수명을 기술한다.
    * */
    @AfterReturning(pointcut = "logPointCut()", returning = "result")
                                                    // returning 의 이름과 동일해야함
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        System.out.println("result 변수에 Service 의 return 값이 담겼는지 확인 : " + result);
        /* comment.
        *   result 변수에 우리가 Service 클래스에서 return 되는 값이 담긴 것을 확인했다.
        *   그렇다는 것은 우리가 result 변수에 접근하여 값을 조작할 수 있다는 의미다. */
    ...
    @AfterThrowing(pointcut = "logPointCut()", throwing = "exception")
                                // // throwing 의 이름과 동일해야함
    public void logAfterThrowing(Throwable exception){
        System.out.println("logAfterThrowing exception" + exception);
    }

    /* comment.
    *   @Around 어노테이션
    *    = Around 는 우리가 지정 한 실행 위치(JoinPoint) 앞 뒤를 모두 장악한다.
    *      그리고 타겟 메소드의 실행 지점, 실행 여부, 계속 진행 여부 등을 결정한다.
    *      Around Advice 는 JoinPoint 를 확장한 ProceedingJoinPoint 를 매개변수로 받게 된다.
    * */
    @Around("logPointCut()")
    public Object logAround(ProceedingJoinPoint joinPoint) throws Throwable {
        /* comment.
        *    앞 뒤로 동작하게 되므로 여기서는 메소드 실행 시
        *   소요되는 시간을 체크하는 로직 작성 */
        ...
        /* comment.
        *   조인 포인트를 호출 한 쪽(지금은 Application) or
        *   다른 어드바이스가 실행 할 수 있도록 Object 반환 */
